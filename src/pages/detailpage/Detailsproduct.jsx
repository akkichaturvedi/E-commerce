...
function Detailsproduct() {
    ...
    const saveProductIntoacart = async () => {
        ...
        const _doc = doc(db, "Products", id)
        const _data = await getDoc(_doc)
        addCartData(_data.data())
    }

    // useEffect(() => {
    //     CartDataaddedByuser.forEach((element) => {
    //         console.log(element);
    //     })
    // }, [CartDataaddedByuser])
                        ...
                        <div className="md:ml-5 px-5 md:mt-0 rounded-lg bg-gray-200  mt-5 flex flex-col border w-full">
                            ...
                            <span>
                                <div className="w-full  mt-5">
                                    <Link to={`/buynowpage/${id}`}><button className="text-2xl m-5 md:ml-0 py-4 px-12 bg-orange-400 text-white
